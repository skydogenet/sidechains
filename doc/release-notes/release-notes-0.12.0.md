...
This allows running testchaind without having to do any manual configuration.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:9325/
        ProxyPassReverse http://127.0.0.1:9325/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass testchaind rpc basic auth
...
Restarting testchaind will no longer clear out the list of banned peers; instead
...
- #6600 `ef4945f` Include testchain-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix testchain-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of testchain-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [testchain-cli] improve error output (Jonas Schnelli)
