...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sidechaind.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sidechain-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/sidechain-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/rpm/bitcoin.te
# Source31 - what about sidechain-tx and bench_bitcoin ???
...
The sidechain-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the sidechain-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/sidechaind %{buildroot}%{_sbindir}/sidechaind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/sidechaind 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/sidechaind/sidechaind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/sidechaind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/sidechaind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/sidechain-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/sidechain-qt.1
%endif
...
%attr(0644,root,root) %{_mandir}/man1/sidechain-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/sidechaind
...
%attr(0644,root,root) %{_mandir}/man1/sidechaind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/sidechain-cli
%attr(0755,root,root) %{_bindir}/sidechain-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/sidechain-cli.1*
