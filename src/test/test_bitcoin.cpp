...
Thunder100Setup::Thunder100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    // Disable default enabled BMM checks for thunder generation - otherwise
...
//
CBlock Thunder100Setup::CreateAndProcessBlock(const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CBlock Thunder100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& vtx, const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CScript Thunder100Setup::GetCoinbaseScript() const
{
    return CScript() << ToByteVector(coinbaseKey.GetPubKey()) << OP_CHECKSIG;
}

Thunder100Setup::~Thunder100Setup()
{
}
